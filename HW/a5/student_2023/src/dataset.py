...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_length = int(random.gauss(truncated_length / 4, 0.3))
        mask_length = max(0, mask_length)
        mask_length = min(truncated_length, mask_length)
        mask_start_idx = random.randint(0, truncated_length - mask_length)
        prefix, masked_content, suffix = (
            truncated_document[:mask_start_idx],
            truncated_document[mask_start_idx : mask_start_idx + mask_length],
            truncated_document[mask_start_idx + mask_length :],
        )
        # print(prefix, masked_content, suffix, sep="\n")
        masked_string = (
