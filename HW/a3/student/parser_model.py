...
class ParserModel(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_to_loggits_weight = nn.Parameter(
            torch.zeros((hidden_size, n_classes))
        )
        self.hidden_to_logits_bias = nn.Parameter(torch.zeros((n_classes)))
        nn.init.xavier_uniform_(self.hidden_to_loggits_weight)
    ...
    def embedding_lookup(self, w: torch.Tensor):
        ...
        x = torch.zeros((len(w), self.n_features * self.embed_size))
    ...
    def forward(self, w: torch.Tensor):
        ...
        logits = h_dropout @ self.hidden_to_loggits_weight + self.hidden_to_logits_bias
