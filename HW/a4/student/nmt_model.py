...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(
            hidden_size, len(self.vocab.tgt), bias=False
        )
    ...
    def encode(
        ...
        init_decoder_hidden = self.h_projection(
            torch.concatenate([last_hidden[0], last_hidden[1]], 1)
        )
        init_decoder_cell = self.c_projection(
            torch.concatenate([last_cell[0], last_cell[1]], 1)
        )
    ...
    def decode(
        ...
        for Y_t in torch.split(Y, 1):
            ...
            Ybar_t = torch.concatenate([Y_t, o_prev], -1)
